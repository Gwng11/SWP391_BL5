...
class ManagerRepositoryIntegrationTest {
    ...
    @Test void checkoutAtomicallyReleasesRoomAndCreatesHousekeepingTask(){service.setRoomTypeActive(roomTypeId,false);new ReservationRepository().checkOut(reservationId,managerId);Room room=service.getRoom(occupiedRoomId);assertEquals("OUT_OF_SERVICE",room.getOperationalStatus());assertEquals("DIRTY",room.getCleaningStatus());assertFalse(room.isActive());HousekeepingTask task=service.getHousekeepingTasks(null,occupiedRoomId,null).stream().filter(t->Long.valueOf(reservationId).equals(t.getReservationId())).findFirst().orElseThrow();assertEquals("CHECKOUT_CLEANING",task.getTaskType());assertEquals("PENDING",task.getStatusCode());assertTrue(new RoomAssignmentRepository().findCurrentByReservation(reservationId).isEmpty());}
