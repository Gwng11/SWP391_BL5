...
class AuthFilterTest {
    ...
    @Test void adminCanAccessAdminRoutesButOthersCannot()throws Exception{when(req.getRequestURI()).thenReturn("/admin/users");when(session.getAttribute(Constants.SESSION_USER)).thenReturn(user(Constants.ROLE_ADMIN));filter.doFilter(req,resp,chain);verify(chain).doFilter(req,resp);reset(chain,dispatcher);when(req.getRequestURI()).thenReturn("/admin/users");when(session.getAttribute(Constants.SESSION_USER)).thenReturn(user(Constants.ROLE_MANAGER));filter.doFilter(req,resp,chain);verify(dispatcher).forward(req,resp);verify(chain,never()).doFilter(req,resp);}
    ...
    @Test void serviceDetailRequiresAuthentication()throws Exception{when(req.getRequestURI()).thenReturn("/service-detail");when(req.getMethod()).thenReturn("GET");when(session.getAttribute(Constants.SESSION_USER)).thenReturn(null);filter.doFilter(req,resp,chain);verify(resp).sendRedirect("/login?redirect=%2Fservice-detail");verify(chain,never()).doFilter(req,resp);}
    @Test void receptionistCanOnlyUseReceptionServiceRequestRoute()throws Exception{when(session.getAttribute(Constants.SESSION_USER)).thenReturn(user(Constants.ROLE_RECEPTIONIST));when(req.getRequestURI()).thenReturn("/reception/service-requests");filter.doFilter(req,resp,chain);verify(chain).doFilter(req,resp);reset(chain,dispatcher);when(req.getRequestDispatcher("/WEB-INF/views/forbidden.jsp")).thenReturn(dispatcher);when(req.getRequestURI()).thenReturn("/staff/service-requests");filter.doFilter(req,resp,chain);verify(dispatcher).forward(req,resp);verify(chain,never()).doFilter(req,resp);}
    @Test void serviceStaffCanOnlyUseStaffServiceRequestRoute()throws Exception{when(session.getAttribute(Constants.SESSION_USER)).thenReturn(user(Constants.ROLE_SERVICE_STAFF));when(req.getRequestURI()).thenReturn("/staff/service-requests");filter.doFilter(req,resp,chain);verify(chain).doFilter(req,resp);reset(chain,dispatcher);when(req.getRequestDispatcher("/WEB-INF/views/forbidden.jsp")).thenReturn(dispatcher);when(req.getRequestURI()).thenReturn("/reception/service-requests");filter.doFilter(req,resp,chain);verify(dispatcher).forward(req,resp);verify(chain,never()).doFilter(req,resp);}
    @Test void adminCannotCoordinateServiceRequests()throws Exception{when(session.getAttribute(Constants.SESSION_USER)).thenReturn(user(Constants.ROLE_ADMIN));for(String path:new String[]{"/reception/service-requests","/staff/service-requests"}){reset(chain,dispatcher);when(req.getRequestDispatcher("/WEB-INF/views/forbidden.jsp")).thenReturn(dispatcher);when(req.getRequestURI()).thenReturn(path);filter.doFilter(req,resp,chain);verify(dispatcher).forward(req,resp);verify(chain,never()).doFilter(req,resp);}}
