...
class AuthControllerTest {
    @Test void managerDestinationIsDashboardAndOtherRolesStayUnchanged(){User u=new User();u.setRoleCode(Constants.ROLE_MANAGER);assertEquals("/manager/dashboard",AuthController.destinationFor(u));u.setRoleCode(Constants.ROLE_RECEPTIONIST);assertEquals("/reception/checkin",AuthController.destinationFor(u));u.setRoleCode(Constants.ROLE_SERVICE_STAFF);assertEquals("/staff/service-requests",AuthController.destinationFor(u));u.setRoleCode(Constants.ROLE_ADMIN);assertEquals("/admin/dashboard",AuthController.destinationFor(u));}
