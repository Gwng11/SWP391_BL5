...
<div class="card" style="max-width:640px">
      ...
      <c:choose>
        ...
        <c:when test="${selectedCustomer != null}">
          <p style="margin:6px 0">
            ✔ <b><c:out value="${selectedCustomer.fullName}"/></b> — ${selectedCustomer.customerCode}
            <c:if test="${not empty selectedCustomer.phone}"> — 📞 <c:out value="${selectedCustomer.phone}"/></c:if>
            <c:if test="${not empty selectedCustomer.idDocumentNumber}"> — ${selectedCustomer.idDocumentType} <c:out value="${selectedCustomer.idDocumentNumber}"/></c:if>
        ...
        <c:otherwise>
          <form method="get" action="${pageContext.request.contextPath}/booking" style="display:flex;gap:8px;flex-wrap:wrap">
            ...
            <input name="q" value="<c:out value='${param.q}'/>" placeholder="SĐT / số CCCD / tên / mã KH" style="width:300px" required>
          ...
          <c:if test="${customerResults != null}">
            <c:choose>
              <c:when test="${not empty customerResults}">
                <table style="margin-top:10px">
                  ...
                  <c:forEach var="c" items="${customerResults}">
                    <tr>
                      <td>${c.customerCode}</td><td><c:out value="${c.fullName}"/></td><td><c:out value="${c.phone}"/></td>
              ...
              <c:otherwise>
                <p style="color:#c0392b;margin:8px 0 4px">Không tìm thấy khách nào khớp "<c:out value="${param.q}"/>".</p>
              </c:otherwise>
            </c:choose>
            ...
            <details style="margin-top:8px" ${empty customerResults ? 'open' : ''}>
              ...
              <form method="post" action="${pageContext.request.contextPath}/booking"
                ...
                <div><label>SĐT</label><input name="newPhone" value="<c:out value='${param.q}'/>" maxlength="30"></div>
  ...
  <form method="post" action="${pageContext.request.contextPath}/booking">
    ...
    <input name="primaryGuestName" value="<c:out value='${selectedCustomer.fullName}'/>" maxlength="150" style="width:100%">
