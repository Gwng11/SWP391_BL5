...
-- F01: hồ sơ khách sạn (bảng 1 dòng)
IF NOT EXISTS (SELECT 1 FROM hotel_profile)
INSERT INTO hotel_profile (profile_id, hotel_name, description, address, phone, email, check_in_time, check_out_time, currency_code)
VALUES (1, N'Sunrise Hanoi Hotel', N'Khách sạn 4 sao ngay trung tâm phố cổ Hà Nội.',
        N'12 Hàng Bài, Hoàn Kiếm, Hà Nội', '+842438252525', 'info@sunrisehotel.vn', '14:00', '12:00', 'VND');
GO

-- Users
IF NOT EXISTS (SELECT 1 FROM users WHERE email = 'admin@hotel.vn')
INSERT INTO users (email, password_hash, full_name, phone, role_code, department_code, status_code, email_verified_at) VALUES
('admin@hotel.vn',        '65536:42o/ESjTo/rXp8yura47jQ==:+uf3pLB8rvMLXty8VDvS1NUafQeZybhoj4e54MhAt6U=', N'Quản trị hệ thống', '0900000001', 'ADMIN', NULL, 'ACTIVE', SYSUTCDATETIME()),
...
IF NOT EXISTS (SELECT 1 FROM customers WHERE email = 'customer@test.vn')
...
-- F21/F02: loại phòng
IF NOT EXISTS (SELECT 1 FROM room_types)
INSERT INTO room_types (type_code, type_name, description, max_adults, max_children, bed_type, room_size_m2, base_price, amenities_json) VALUES
...
INSERT INTO rooms (room_type_id, room_number, floor_number)
...
JOIN room_types rt ON rt.type_code = v.type_code;
GO

-- F15: dịch vụ
IF NOT EXISTS (SELECT 1 FROM hotel_services)
INSERT INTO hotel_services (service_code, service_name, description, unit_name, unit_price) VALUES
...
-- F26: email templates cho các event dùng ở F04, F06-F08, F14
IF NOT EXISTS (SELECT 1 FROM email_templates)
INSERT INTO email_templates (template_code, template_name, event_code, subject_template, body_html) VALUES
('TPL_VERIFY', N'Xác thực tài khoản', 'ACCOUNT_VERIFICATION',
 N'Xác thực tài khoản Sunrise Hotel',
 N'<p>Chào {{full_name}},</p><p>Bấm vào link sau để xác thực email: <a href="{{verify_link}}">Xác thực ngay</a></p><p>Link có hiệu lực 24 giờ.</p>'),
('TPL_RESET', N'Đặt lại mật khẩu', 'PASSWORD_RESET',
 N'Đặt lại mật khẩu Sunrise Hotel',
 N'<p>Chào {{full_name}},</p><p>Bấm vào link sau để đặt lại mật khẩu: <a href="{{reset_link}}">Đặt lại mật khẩu</a></p><p>Link có hiệu lực 60 phút.</p>'),
('TPL_RES_CONFIRM', N'Xác nhận đặt phòng', 'RESERVATION_CONFIRMED',
 N'Đặt phòng thành công - Mã {{booking_code}}',
 N'<p>Chào {{full_name}},</p><p>Đơn <b>{{booking_code}}</b> ({{check_in_date}} → {{check_out_date}}) đã được ghi nhận.</p><p>Tổng tiền: {{total_amount}} đ. Vui lòng đặt cọc {{deposit_required}} đ để xác nhận.</p>'),
('TPL_RES_UPDATE', N'Cập nhật đặt phòng', 'RESERVATION_UPDATED',
 N'Đơn {{booking_code}} đã được cập nhật',
 N'<p>Chào {{full_name}},</p><p>Đơn <b>{{booking_code}}</b> đã đổi thành {{check_in_date}} → {{check_out_date}}. Tổng mới: {{total_amount}} đ.</p>'),
('TPL_RES_CANCEL', N'Hủy đặt phòng', 'RESERVATION_CANCELLED',
 N'Đơn {{booking_code}} đã hủy',
 N'<p>Chào {{full_name}},</p><p>Đơn <b>{{booking_code}}</b> đã được hủy. Lý do: {{cancellation_reason}}</p>'),
('TPL_DEPOSIT', N'Biên nhận đặt cọc', 'DEPOSIT_RECEIPT',
 N'Biên nhận đặt cọc - Đơn {{booking_code}}',
 N'<p>Chào {{full_name}},</p><p>Đã nhận {{amount}} đ tiền cọc cho đơn <b>{{booking_code}}</b> ({{method}}, mã GD: {{reference}}).</p>'),
('TPL_INVOICE', N'Hóa đơn & biên nhận', 'INVOICE_AND_RECEIPT',
